...
Automated processing of access requests to sensitive data is one of the major bottlenecks facing automated research workflows because access conditions and limitations are not systematically encoded in licences and policies. As a result, requests almost always have to be adjudicated by a human agent: this is costly and slow. Moreover, sensitive data is often assumed to mean only personal data, given the prominence of GDPR in Europe. There are, however, many other categories of sensitive data, including data that is commercially, environmentally, and culturally sensitive or has security implications. 

...
4. A *Ticket* encodes specific rules in such a way that it can be used in practical terms - for example, defining the number of times an Asset may be accessed, and being updated every time that an access event occurs.
...
- Each of these Parties Party Collections can, in practice, express these Assertions in specific ways:
 ...
 - End Users can assert context or credentials, for example that they are a researcher or affiliated with a research institution.
